  ...
  return (
    <div className="bg-[#faf5ff] dark:bg-[#0d0f1a] flex items-center justify-center p-6 min-h-[calc(100vh-136px)] transition-colors duration-300">
      <div className="w-full max-w-5xl flex flex-col lg:flex-row rounded-3xl overflow-hidden shadow-2xl">
        ...
        {/* Left - Form */}
        <div className="w-full lg:w-1/2 flex flex-col justify-center px-8 md:px-16 py-12 bg-white dark:bg-[#111827]">

          {/* Logo */}
          <div className="flex items-center gap-2 mb-10">
            <div className="w-10 h-10 bg-gradient-to-br from-[#295AFC] to-[#9b59ff] rounded-xl flex items-center justify-center">
              <svg className="w-5 h-5 fill-white" viewBox="0 0 24 24">
                <path d="M12 3L4 7v5c0 5.25 3.4 10.15 8 11.35C16.6 22.15 20 17.25 20 12V7l-8-4z" />
              </svg>
            </div>
            <span className="text-[#6d4cff] font-bold text-lg">Ninja Teacher</span>
          </div>

          <h1 className="text-3xl font-bold text-gray-900 dark:text-[#f1f0ff] mb-1">Welcome Back! 👋</h1>
          <p className="text-gray-500 dark:text-[#9d94c4] text-sm mb-8">Continue your journey with us</p>

          {/* Teacher / School Toggle */}
          <div className="flex bg-gray-100 dark:bg-[#1a2236] rounded-2xl p-1 mb-8">
            <button
              onClick={() => setActiveTab("teacher")}
              className={`flex-1 flex items-center justify-center gap-2 py-3 rounded-xl text-sm font-medium transition-all ${activeTab === "teacher"
                  ? "bg-gradient-to-br from-[#295AFC] to-[#9b59ff] text-white shadow-lg shadow-[#295AFC]/30"
                  : "text-gray-500 dark:text-[#9d94c4] hover:text-gray-700 dark:hover:text-[#e8e4ff]"
                }`}
            >
            ...
            <button
              ...
              className={`flex-1 flex items-center justify-center gap-2 py-3 rounded-xl text-sm font-medium transition-all ${activeTab === "school"
                  ? "bg-gradient-to-br from-[#295AFC] to-[#9b59ff] text-white shadow-lg shadow-[#295AFC]/30"
                  : "text-gray-500 dark:text-[#9d94c4] hover:text-gray-700 dark:hover:text-[#e8e4ff]"
                }`}
            >
          ...
          <div className="space-y-4">
            ...
            <form onSubmit={handleSubmit(submitForm)} className="space-y-5">
              ...
              <button
                ...
                className="w-full bg-gradient-to-br from-[#295AFC] to-[#9b59ff] hover:from-[#1f4df5] hover:to-[#8d4dff] text-white font-medium py-3.5 rounded-2xl text-sm transition-all flex items-center justify-center gap-2 disabled:opacity-70 disabled:cursor-not-allowed"
              >
                {isSubmitting ? (
            ...
            <div className="flex items-center gap-4 py-4">
              <div className="flex-1 h-px bg-gray-100 dark:bg-[#1f2937]"></div>
              <span className="text-xs text-gray-400 dark:text-[#6b7280] font-medium uppercase tracking-wider">Or continue with</span>
              <div className="flex-1 h-px bg-gray-100 dark:bg-[#1f2937]"></div>
            </div>

            {/* Social */}
            <div className="flex gap-3">
              <button type="button" className="flex-1 flex items-center justify-center gap-2 border border-gray-200 dark:border-[#2d3748] rounded-xl py-2.5 text-sm font-medium text-gray-700 dark:text-[#c4b5fd] hover:bg-gray-50 dark:hover:bg-[#1a2236] transition-colors">
        ...
        {/* Right - Banner (gradient — يفضل زي ما هو في الحالتين) */}
        <div className="hidden lg:flex w-1/2 bg-gradient-to-br from-[#295AFC] to-[#9b59ff] flex-col items-center justify-center px-16 text-white">
          ...
          {activeTab === "teacher" ? (
            <>
              <h2 className="text-3xl font-bold text-center mb-4 leading-tight">Your Dream Teaching<br />Job Awaits</h2>
              <p className="text-white/70 text-center text-sm mb-10 leading-relaxed">AI-powered matching connects you with schools<br />that value your unique teaching style</p>
              <div className="grid grid-cols-3 gap-4 w-full mb-10">
                {[{ icon: "👥", num: "1,200+", label: "Teachers" }, { icon: "💼", num: "500+", label: "Jobs" }, { icon: "🏆", num: "95%", label: "Success" }].map((s) => (
              ...
              <ul className="space-y-3 w-full">
                {["Personality-based matching algorithm", "Verified teaching opportunities", "Direct communication with schools"].map((f) => (
                  <li key={f} className="flex items-center gap-3 text-sm text-white/80">
                    <span className="w-2 h-2 bg-white rounded-full shrink-0"></span>{f}
                  </li>
                ))}
              </ul>
            </>
            ...
            <>
              <h2 className="text-3xl font-bold text-center mb-4 leading-tight">Find Your Perfect<br />Teacher Match</h2>
              <p className="text-white/70 text-center text-sm mb-10 leading-relaxed">AI-powered matching helps you find teachers<br />that fit your school's culture and needs</p>
              <div className="grid grid-cols-3 gap-4 w-full mb-10">
                {[{ icon: "🏫", num: "300+", label: "Schools" }, { icon: "🎓", num: "1,200+", label: "Teachers" }, { icon: "⭐", num: "98%", label: "Satisfied" }].map((s) => (
                  <div key={s.label} className="bg-white/15 rounded-2xl p-4 text-center">
                    <div className="text-2xl mb-1">{s.icon}</div>
                    <div className="text-lg font-bold">{s.num}</div>
                    <div className="text-white/60 text-xs">{s.label}</div>
